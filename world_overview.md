...
- Personality lives in seven dimensions [0,1]: aggression, cooperation, wealth_focus, food_focus, risk_tolerance, trust_in_others, adaptability. The neutral baseline is 0.5 on each.
- Trait catalogue (max two active, cooldown enforced between changes):
  - **Aggressive** (incompatible with `Friendly`): pushes self-favouring deals, high aggression/risk, low trust/cooperation.
  - **Friendly** (incompatible with `Aggressive`/`Isolationist`): favours balanced outcomes, high cooperation/trust, low aggression.
  - **Wealth-hoarder**: high wealth_focus, lower risk, cautious trust; reluctant to spend wealth.
  - **Food-secure**: high food_focus, low risk; prioritises food safety.
